...
import type { Gateway, GatewayBot, RawGateway, RawGatewayBot } from '#types/index.js';
...
export class GatewayResource extends ResourceBase {
	...
	async get(): Promise<Gateway> {
		const { rest } = this;
		const gatewayResponseData = await rest.makeRequest<RawGateway>(GATEWAY_ENDPOINT(), {
	...
	async getBot(): Promise<GatewayBot> {
		...
		const gatewayBotResponseData = await rest.makeRequest<RawGatewayBot>(GATEWAY_BOT_ENDPOINT(), {
			method: RESTMethod.Get,
		});
