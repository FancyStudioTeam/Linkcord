...
describe("Class: BitFieldResolver", () => {
  ...
  describe("Method: remove", () =>
    it("Removes the given bit from the bitfield.", () => {
      ...
      expect(
        new BitFieldResolver(UserFlags.ActiveDeveloper | UserFlags.Staff).remove(UserFlags.Staff),
  ...
  describe("Method: has", () =>
    it("Checks whether the given bit is present in the bitfield.", () => {
      ...
      expect(new BitFieldResolver(UserFlags.ActiveDeveloper).has(UserFlags.ActiveDeveloper)).toBe(
        true,
      );
      expect(
        new BitFieldResolver(UserFlags.ActiveDeveloper | UserFlags.Staff).has(UserFlags.Staff),
      ).toBe(true);
    }));
