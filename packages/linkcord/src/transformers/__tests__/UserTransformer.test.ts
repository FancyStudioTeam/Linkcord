...
describe("Class: UserTransformer", () => {
  it("Transforms a basic raw user into a 'User' object and returns the transformed user object.", () => {
    ...
    const transformedUser = UserTransformer.transformFromRawUser({
      ...
      flags,
      /**
       * biome-ignore lint/style/useNamingConvention: Discord properties are
       * snake cased.
       */
    ...
    expect(transformedUser).toMatchObject<UserProperties>({
      ...
      bot: false,
      collectibles: {},
      createdAt: new Date(DiscordSnowflake.timestampFrom("1200870671681605662")),
      discriminator: "0",
    ...
    expect(transformedUser.avatarUrl()).toBe(
      "https://cdn.discordapp.com/avatars/1200870671681605662/d259b2b5deb475b5928c2f73a8ce6101.png",
    );
    expect(transformedUser.bannerUrl()).toBe(null);
    expect(transformedUser.defaultAvatarUrl()).toBe("https://cdn.discordapp.com/embed/avatars/3.png");
  });
});
